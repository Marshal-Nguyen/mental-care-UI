...
export const TherapeuticActivityQuestion = React.forwardRef(
  (
    ...
    // Fetch therapeutic activities for the current page
    const fetchActivities = useCallback(async (page) => {
      try {
        setIsFetching(page === 1);
        setIsLoadingMore(page > 1);
        const response = await fetch(
          `https://api.emoease.vn/lifestyle-service/therapeutic-activities?PageIndex=${page}&PageSize=10`,
          {
            headers: {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch therapeutic activities");
        }
        const data = await response.json();
        console.log("API response:", data); // Debug: Log full API response
        const newActivities = data.therapeuticActivities.data
          .filter(
          ...
          .map((activity) => ({
            ...
            instructions: activity.instructions || "Không có hướng dẫn",
        ...
        setTotalPages(data.therapeuticActivities.totalPages);
        setIsFetching(false);
        setIsLoadingMore(false);
      } catch (err) {
        console.error("Error fetching therapeutic activities:", err);
...
const getActivityIcon = (name) => {
  if (!name || typeof name !== "string") return "🧘"; // Fallback if name is undefined or not a string
  const lowerName = name.toLowerCase();
  if (lowerName.includes("meditation") || lowerName.includes("mindfulness"))
    return "🧘";
  if (lowerName.includes("behavioral") || lowerName.includes("activation"))
    return "🚶";
  if (lowerName.includes("journaling") || lowerName.includes("writing"))
    return "📝";
  if (lowerName.includes("art") || lowerName.includes("drawing")) return "🎨";
  if (lowerName.includes("music") || lowerName.includes("listening"))
    return "🎶";
  return "🧘";
};
