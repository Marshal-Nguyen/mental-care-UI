    ...
    const fetchActivities = useCallback(async (page) => {
      try {
        ...
        const response = await fetch(
          `https://api.emoease.vn/lifestyle-service/entertainment-activities?PageIndex=${page}&PageSize=10`,
          {
            headers: {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch entertainment activities");
        }
        const data = await response.json();
        const newActivities = data.entertainmentActivities.data.map(
          (activity) => ({
            value: activity.id,
            label: activity.name,
        ...
        setTotalPages(data.entertainmentActivities.totalPages);
        setIsFetching(false);
      ...
      return (
        <motion.div
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-white mt-3 text-sm">Đang tải hoạt động...</p>
          </div>
        </motion.div>
      );
    }
      ...
      <motion.div
        ...
        <motion.h1
          ...
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-purple-200">
            Hành động vui chơi giải trí mà bạn thích là gì?
          </span>
          <motion.div
