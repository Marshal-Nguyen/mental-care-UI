...
const ProfileDoctor = () => {
  ...
  const fetchPatientName = async (patientId) => {
    try {
      const response = await axios.get(
        `https://api.emoease.vn/profile-service/patients/${patientId}`
      );
  ...
  const fetchBookings = async (doctorId, status) => {
    try {
      let url = `https://api.emoease.vn/scheduling-service/bookings?PageIndex=1&PageSize=30&DoctorId=${doctorId}`;
      if (status !== "All") {
        url += `&Status=${status}`;
      }
  ...
  useEffect(() => {
    const fetchDoctorData = async () => {
      try {
        ...
        await Promise.all([
          fetchAvatar(doctorId),
          fetchBookings(userId, selectedStatus),
        ]);
        setLoading(false);
      } catch (err) {
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {bookings.map((booking) => (
                      <tr key={booking.bookingCode}>
                        ...
                        <td
                          className={`px-6 py-4 whitespace-nowrap text-sm font-bold ${getStatusColor(
                            booking.status
                          )}`}>
                          {booking.status}
                        </td>
