    ...
    const fetchCustomer = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.emoease.vn/profile-service/patients/${id}`
        );
        ...
        const imageResponse = await fetch(
          `https://api.emoease.vn/image-service/image/get?ownerType=User&ownerId=${data.patientProfileDto.userId}`
        );
        setProfileImage(
          imageResponse.ok
        ...
        const subscriptionResponse = await fetch(
          `https://api.emoease.vn/subscription-service/service-packages?PageIndex=1&PageSize=10&patientId=${id}`
        );
        if (!subscriptionResponse.ok)
          throw new Error("Failed to fetch subscription data");
        const subscriptionData = await subscriptionResponse.json();
        const purchasedPackage = subscriptionData.servicePackages.data.find(
          (pkg) => pkg.isPurchased
        );
        setPurchasedPackageName(
          purchasedPackage ? purchasedPackage.name : null
        );
      } catch (error) {
