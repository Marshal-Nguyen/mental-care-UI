    ...
    const fetchBookingDetails = async () => {
      try {
        ...
        const bookingResponse = await axios.get(
          `https://api.emoease.vn/scheduling-service/bookings/${id}`
        );
        ...
        const doctorResponse = await axios.get(
          `https://api.emoease.vn/profile-service/doctors/${bookingData.doctorId}`
        );
        ...
        const patientResponse = await axios.get(
          `https://api.emoease.vn/profile-service/patients/${bookingData.patientId}`
        );
        ...
        const doctorImageResponse = await axios.get(
          `https://api.emoease.vn/image-service/image/get?ownerType=User&ownerId=${doctorData.userId}`
        );
        setDoctorImage(doctorImageResponse.data.url);

        // Fetch patient image
        const patientImageResponse = await axios.get(
          `https://api.emoease.vn/image-service/image/get?ownerType=User&ownerId=${patientData.userId}`
        );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        {doctor && (
          <div className="bg-white rounded-xl shadow-2xl p-6 border-t-4 border-blue-500">
            <h2 className="text-2xl font-semibold mb-4 text-blue-700 flex items-center">
              <FaBriefcase className="h-6 w-6 mr-2 text-blue-600" /> Doctor
              Information
            </h2>
            {doctorImage && (
            ...
            <div className="space-y-4">
              <InfoItem
                ...
                icon={<AiOutlineUser className="h-5 w-5 text-blue-500" />}
              />
              <InfoItem
                label="Gender"
                value={doctor.gender}
                icon={<FaUserCircle className="h-5 w-5 text-purple-500" />}
              />
              <InfoItem
                label="Contact"
                value={doctor.contactInfo.phoneNumber}
                icon={<AiOutlinePhone className="h-5 w-5 text-blue-500" />}
              />
              <InfoItem
                label="Specialties"
              ...
              <InfoItem
                ...
                icon={<AiOutlineStar className="h-5 w-5 text-yellow-500" />}
              />
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow-2xl p-6 border-t-4 border-purple-500">
          <h2 className="text-2xl font-semibold mb-4 text-purple-700 flex items-center">
            <ClockIcon className="h-6 w-6 mr-2 text-purple-600" /> Booking
            Information
          </h2>
          <div className="space-y-4">
        ...
        {patient && (
          <div className="bg-white rounded-xl shadow-2xl p-6 border-t-4 border-pink-500">
            <h2 className="text-2xl font-semibold mb-4 text-pink-700 flex items-center">
              <FaHeart className="h-6 w-6 mr-2 text-pink-600" /> Patient
              Information
            </h2>
            {patientImage && (
              <div className="flex justify-center mb-6">
            ...
            <div className="space-y-4">
              <InfoItem
                ...
                icon={<AiOutlineUser className="h-5 w-5 text-pink-500" />}
              />
              <InfoItem
                label="Gender"
              ...
              <InfoItem
                ...
                icon={<FaPhone className="h-5 w-5 text-pink-500" />}
              />
              <InfoItem
                label="Allergies"
              ...
              <InfoItem
                ...
                icon={<MdPsychology className="h-5 w-5 text-blue-500" />}
              />
              <InfoItem
