    ...
    const fetchCustomers = async () => {
        try {
            ...
            const response = await axios.get(BASE_API_URL, {
                params: { PageIndex: pageIndex, PageSize: pageSize, Search: searchQuery, SortBy: sortBy, SortOrder: sortOrder },
            });
            ...
            const customersWithImages = await Promise.all(
                response.data.paginatedResult.data.map(async (customer) => {
                    try {
                        ...
                        return { ...customer, profileImage: imageResponse.data.url || "https://cdn-healthcare.hellohealthgroup.com/2023/05/1684813854_646c381ea5d030.57844254.jpg?w=1920&q=100" };
                    } catch (imgError) {
                        return { ...customer, profileImage: "https://cdn-healthcare.hellohealthgroup.com/2023/05/1684813854_646c381ea5d030.57844254.jpg?w=1920&q=100" };
                    }
                })
            );
    ...
    return (
        <div className="container mx-auto p-6 mt-2 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            ...
            >
                ...
                <h2 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600">Users Dashboard</h2>
            </motion.div>
                        ...
                        <tbody>
                            {customers.map((customer, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 text-gray-600 font-medium">{customer.contactInfo?.phoneNumber || "N/A"}</td>
                                    <td
